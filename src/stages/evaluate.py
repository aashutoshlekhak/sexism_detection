...
## TODO: 1. Map class labels to prediction label
        # 2. Save the reports in proper directory 
...
def evaluate(config_path:str, data_name:str, data_type:str, model_ckpt:str, fold:int, task:Optional[str])->None:
    ...
    for out in tqdm(pipe(KeyDataset(test_data, "text"), batch_size=16, truncation=True), total=len(test_data)):
        ...
        predicted_label_index = int(predicted_label.split()[-1])
        predictions.append(predicted_label_index)
    

    if task:
        metric_dir = os.path.join("src/metrics", model_ckpt, data_type,data_name,task,str(fold))
    else:
        metric_dir = os.path.join("src/metrics", model_ckpt, data_type,data_name, str(fold))
    ...
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', cbar=False, xticklabels=['non-sexist', 'sexist'], yticklabels=['non-sexist', 'sexist'])
...
if __name__ == "__main__":
    ...
    evaluate(args.config, args.data_name, args.data_type,args.model, int(args.fold),args.task) 